...
class SetSecurityKey : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_set_securitykey)

        val emailField =findViewById<EditText>(R.id.emailID)
        val secretKeyField = findViewById<EditText>(R.id.secretKey)
        val repeatKeyField = findViewById<EditText>(R.id.repeatKey)
        val setKeyBtn = findViewById<Button>(R.id.Set_Key)
        val dashboardLink = findViewById<TextView>(R.id.DashboardLink)

        // Navigate to Dashboard
        dashboardLink.setOnClickListener{v : View ->
            startActivity(Intent(v.context, Dashboard::class.java))
        }
    ...
    private fun checkEmail(email: String): Boolean {
        if (TextUtils.isEmpty(email) || !android.util.Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            findViewById<EditText>(R.id.emailID).error = "Please enter a valid email address."
            return false
        }
        else return true
    }
    ...
    private fun checkSecretKey(secretKeyVal: String, repeatKeyVal: String): Boolean {
        if (secretKeyVal.isEmpty()) {
            findViewById<EditText>(R.id.secretKey).error = "Please enter a secret key."
            return false
        }
        else if (secretKeyVal.length < 6) {
            findViewById<EditText>(R.id.secretKey).error = "Please enter a secret key with more than six characters."
            return false
        }
        else if (secretKeyVal != repeatKeyVal) {
            findViewById<EditText>(R.id.repeatKey).error = "Keys do not match."
            return false
        }
        else return true
    }
